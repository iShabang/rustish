...
TEST_CASE("is_some_and returns true on a full Option with a true predicate",
          "[value]") {
    int actual = 0;
    REQUIRE(Some(5).is_some_and([&](int &&val) {
        actual = val;
        return true;
    }));
    REQUIRE(actual == 5);
...
TEST_CASE("unwrap_or_default returns a default value on an empty Option",
          "[value]") {
...
TEST_CASE("map_or computes a new value", "[value]") {
    int actual = 0;
    Option<double> a = Some(5).map_or(1.0, [&](int &&val) {
        actual = val;
        return 2.0;
    });
    REQUIRE(a.is_some());
...
TEST_CASE("map_or returns the default value", "[value]") {
    int actual = 0;
    Option<double> a = Option<int>().map_or(1.0, [&](int &&val) {
        actual = val;
        return 2.0;
    });
    REQUIRE(a.is_some());
...
TEST_CASE("map_or_else computes the default value", "[value]") {
    int actual = 0;
    Option<double> a = Option<int>().map_or_else([]() { return 1.0; },
                                                 [&](int &&val) {
                                                     actual = val;
                                                     return 2.0;
                                                 });
...
TEST_CASE("map_or_else computes a new value", "[value]") {
    int actual = 0;
    Option<double> a = Some(5).map_or_else([]() { return 1.0; },
                                           [&](int &&val) {
                                               actual = val;
                                               return 2.0;
                                           });
...
TEST_CASE("and_then returns a new full Option", "[value]") {
    int actual = 0;
    Option<double> a = Some(5).and_then([&](int &&val) {
        actual = val;
        return Some(2.0);
    });
    REQUIRE(a.is_some());
...
TEST_CASE("filter returns the same Option when given a true predicate",
          "[value]") {
    int actual = 0;
    Option<int> a = Some(5).filter([&](const int &val) {
        actual = val;
        return true;
    });
    REQUIRE(a.is_some());
