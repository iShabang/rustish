...
TEST_CASE("is_some_and returns false on an empty Option with a true predicate",
          "[const-ref]") {
    REQUIRE(!Option<const int &>().is_some_and(
        [](const int &val) { return true; }));
}
...
TEST_CASE("as_ref returns an empty Option from an empty Option",
          "[const-ref]") {
...
TEST_CASE("as_mut returns an empty Option from an empty Option",
          "[const-ref]") {
    Option<const int &> a = None();
    Option<const int &> b = a.as_mut();
    REQUIRE(b.is_none());
...
TEST_CASE("unwrap_or returns the other value on an empty Option",
          "[const-ref]") {
    int other = 6;
    REQUIRE(&Option<const int &>().unwrap_or(other) == &other);
}

TEST_CASE("unwrap_or_else returns other value on a full Option",
          "[const-ref]") {
    const int value = 5;
    int other = 6;
...
TEST_CASE("unwrap_or_else returns the other value on an empty Option",
          "[const-ref]") {
    int other = 6;
    REQUIRE(&Option<const int &>().unwrap_or_else(
                [&]() -> int & { return other; }) == &other);
}

TEST_CASE("unwrap_unchecked returns the value on a full Option",
          "[const-ref]") {
    const int value = 5;
...
TEST_CASE("map returns a new Option", "[const-ref]") {
    ...
    SECTION("new reference type") {
        double newVal = 2.0;
        Option<double &> b =
            a.map([&](const int &val) -> double & { return newVal; });
        REQUIRE(b.is_some());
        REQUIRE(!b.is_none());
...
TEST_CASE("and_then returns an empty Option", "[const-ref]") {
    Option<double> a = Option<const int &>().and_then(
        [](const int &val) { return Some(2.0); });
    REQUIRE(a.is_none());
    REQUIRE(!a.is_some());
}
...
TEST_CASE("or_else computes the new Option", "[const-ref]") {
    const int other = 4;
    Option<const int &> a =
        Option<const int &>().or_else([&]() { return Some(other); });
    REQUIRE(a.is_some());
    REQUIRE(!a.is_none());
    REQUIRE(&a.unwrap_unchecked() == &other);
}
...
TEST_CASE("get_or_insert_with returns existing value", "[const-ref]") {
    ...
    Option<const int &> a = Some(initial);
    const int &val =
        a.get_or_insert_with([&]() -> const int & { return newVal; });
    REQUIRE(&val == &initial);
    REQUIRE(a.is_some());
    REQUIRE(!a.is_none());
}
